...
**Data flow for chat:** `/api/chats` and `/api/chats/[id]/messages` read **Supabase `chat_histories`**, not Chatwoot — that table is also the n8n sales agent's LangChain memory (Postgres Chat Memory node). Any row inserted there **must** carry `message.type` (`'ai'`/`'human'`): LangChain throws `Got unexpected type: undefined` on a row without it, which kills the agent for that contact permanently. Always build the row with `buildChatHistoryMessage()` (`src/lib/chatHistory.ts`), never inline. The Chatwoot description below applies to the legacy path (`/api/chats/search`, `/api/agents`).

`useChats` → `/api/chats` → Chatwoot `/conversations`. Chatwoot's response shape is `data.data.payload` and it **ignores `per_page`** (always ~25 items/page). Phone numbers arrive in several formats (`+5492215...`, `549221...@s.whatsapp.net`, `WAID:549221...`, `wa_id`, `meta.sender.identifier`) — normalize with `extractNumericPhone()` and compare **last 10 digits** to match across formats. If a chat isn't in the locally cached page, `ChatList.js` falls back to `POST /api/chats/search` which paginates the full Chatwoot history.
